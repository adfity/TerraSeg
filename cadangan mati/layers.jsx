...
export default function LayersPanel({ activeLayers, onToggleLayer, rbiData, onToggleProvinsi, fetchRbiData }) {
  ...
  const stats = useMemo(() => {
    ...
    return counts;
  }, [rbiData, activeTab, currentCat]);

  return (
    <div className="flex flex-col h-full bg-white select-none">
      {/* HEADER */}
      <div className="p-4 bg-slate-900 text-white">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-3">Kategori Sarana</p>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/50 p-4 space-y-4">
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-xs font-black text-slate-800 uppercase tracking-tight">{currentCat.label}</h4>
