...
import LayersPanel, { BoundaryLayer, useBoundaryData } from './panel/layers';
...
// Konfigurasi Kategori untuk Map.js
...
export default function Map({ activePanel, setActivePanel }) {
  ...
  // Gunakan custom hook dari layers.jsx untuk batas wilayah
  const { boundaryData, getBoundaryStyle, onEachBoundary } = useBoundaryData(activeLayers);

  // Fungsi untuk fetch data RBI
  ...
  // Fetch data RBI saat activeLayers berubah
  useEffect(() => {
    activeLayers.forEach(layerId => {
      // Skip layer batas wilayah (sudah dihandle oleh useBoundaryData)
  ...
  return (
    <div className="h-screen w-full relative overflow-hidden bg-slate-900">
      ...
      >
        {/* Render Batas Wilayah menggunakan component dari layers.jsx */}
        <BoundaryLayer 
          activeLayers={activeLayers}
